...
const getOrderedQty = (
  ...
  const studentIds = students
    .filter((student) => teacherIds.includes(student.teacherId))
...
const getMealTime = (teacher: User, date: string) => {
  switch (DateTimeUtils.toDate(date).getDay()) {
    case 1:
      return teacher.mondayLunchTime;
    case 2:
      return teacher.mondayLunchTime;
    case 3:
      return teacher.mondayLunchTime;
...
const CafeteriaReport = React.forwardRef<HTMLDivElement, CafeteriaReportProps>(
  (props, ref) => {
    ...
    const mealTimes = Array.from(
      new Set(users.filter(user => user.role === Role.TEACHER).map((teacher) => getMealTime(teacher, props.date)))
    ).sort();
