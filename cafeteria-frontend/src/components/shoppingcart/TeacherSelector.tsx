...
const TeacherSelector: React.FC<TeacherSelectorProps> = ({
  ...
  useEffect(() => {
      ...
      .filter(
        (u) => {
          const teacherLunchTimes = schoolYear.teacherLunchTimes.filter(
            (lt) => lt.teacherId === u.id && lt.dayOfWeek === dayOfWeek
          );
          return u.id ===
          teacherLunchTimes.find(
            (lt) =>
              lt.grades.length &&
              lt.grades[0] === studentGradeLevel &&
              lt.times.length > 0
          )?.teacherId
    ...
    let studentsTeacher = !student?.id
      ? undefined
      : gradeLevelTeachers.find(
          (u) =>
            u.id ===
            schoolYear.studentLunchTimes.find(
              (lt) => lt.studentId === student?.id && lt.dayOfWeek === dayOfWeek
    ...
    if (studentsTeacher) {
      onTeacherSelected(studentsTeacher.id);
    }
  ...
  return (
    <FormControl variant="standard" sx={{ minWidth: "150px" }}>
      <InputLabel id="order-teacher-label">
        {grade !== undefined ? "Lunchtime Teacher" : DAY_NAMES[dayOfWeek]}
      </InputLabel>
      <Select
        ...
        variant="standard"
        disabled={
          selectedTeacher !== undefined && availableTeachers.length == 1
        }
      ...
      >
        ...
        {availableTeachers.map((teacher) => (
          <MuiMenuItem key={teacher.id} value={teacher.id.toString()}>
            {teacher.name.length > 0 ? teacher.name : teacher.firstName + " " + teacher.lastName}
          </MuiMenuItem>
        ))}
      </Select>
    </FormControl>
