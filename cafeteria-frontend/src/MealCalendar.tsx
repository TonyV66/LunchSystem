...
import DateTimeSelectionDialog from "./components/DateTimeSelectionDialog";
import { RelativeDateCountType, RelativeDateTarget } from "./AdminSettingsPage";
import DailyMealsDialog from "./DailyMealsDialog";
import AddToCartDialog from "./AddToCartDialog";
import { Role } from "./models/User";
import CafeteriaDialog from "./CafeteriaDialog";
import ClassroomMealsDialog from "./ClassroomMealsDialog";
...
const AdminMealButtons: React.FC<AdminMealButtonProps> = ({
  ...
  const { scheduledMenus, setScheduledMenus, systemDefaults, orders } =
  ...
  const calculateOrderStartTime = (mealDate: Date) => {
    return calculateOrderTime(
      mealDate,
      systemDefaults.orderStartPeriodCount,
      systemDefaults.orderStartPeriodType,
      systemDefaults.orderStartRelativeTo,
      systemDefaults.orderStartTime
    );
  };
  ...
  const calculateOrderEndTime = (mealDate: Date) => {
    return calculateOrderTime(
      mealDate,
      systemDefaults.orderEndPeriodCount,
      systemDefaults.orderEndPeriodType,
      systemDefaults.orderEndRelativeTo,
      systemDefaults.orderEndTime
    );
  };
...
const TeacherMealButtons: React.FC<CafeteriaMealButtonProps> = ({
  ...
  const studentIds = students
    .filter((student) => student.teacherId === user.id)
...
const MonthlyMealPlan: React.FC<MonthlyMealPlanProps> = ({
  ...
  return (
    <Accordion defaultExpanded={expand}>
      ...
      <AccordionDetails>
        <Box sx={{ pb: 1, pt: 1, overflowX: 'auto', display: "flex", flexDirection: "column", gap: 1 }}>
          {dates.map((date) =>
            (
              <WeeklyMealPlan
                key={date}
